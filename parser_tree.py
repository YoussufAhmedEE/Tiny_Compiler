...
class ScrollableCanvas(tk.Frame):
    ...
    def draw_node_shape(self, x, y, node_type, size=60):
        ...
        if node_type in ["op", "factor","id","const"]:
        ...
        def draw_subtree(node, x, y, total_width):
            ...
            def draw_child_list(child_list, child_x, child_y):
                ...
                for idx, child in enumerate(child_list):
                    ...
                    if not child.right and not child.center and not child.left:
                    ...
                    if idx < len(child_list) - 1:
                        ...
                        self.canvas.create_line(prev_x + node_size // 2, child_y, prev_x + (child_right_width + child_center_width) + next_left_width + next_center_width // 2 - node_size // 2 - gap, child_y, arrow="none")
                        prev_x += next_left_width + next_center_width // 2
                    prev_x += (child_right_width + child_center_width) - gap

            # Handle left child
            if node.left:
                # child_center_width = calculate_width_general(node.center)
                child_center_width = calculate_width_general(node.center[0]) // 2 if isinstance(node.center, list) else calculate_width_general(node.center) // 2

                if isinstance(node.left, list) and len(node.left) >= 2:
                    child_x = x - left_width - child_center_width + calculate_width_general(node.left[0]) // 2 #current_x + left_width // 2
                    draw_child_list(node.left, child_x, child_y)
                    if node.type:
                        self.canvas.create_line(x, y + node_size // 2, child_x, child_y - node_size // 2, arrow="none")
                else:
                    if isinstance(node.left, list):
                        node.left = node.left[0]
                    child_x = x - child_center_width - left_width // 2 #current_x + left_width // 2
            ...
            # Handle right child
            if node.right:
                center_width -= calculate_width_general(node.center[0]) // 2 if isinstance(node.center, list) else calculate_width_general(node.center) // 2
